 ...
 *
 * Nút "Người dùng nội bộ / bên ngoài" chỉ lọc danh sách tài khoản thử nghiệm gợi ý
 * theo actorType — việc phân luồng nội bộ/doanh nghiệp thật sự vẫn do actorType của
 * tài khoản tìm thấy quyết định (xem App.tsx: onAuthenticated), không do nút này.
 */
...
export const LoginScreen: React.FC<LoginScreenProps> = ({ users, policy, onAuthenticated }) => {
  ...
  const demoUsers = users.filter((u) =>
    userType === 'INTERNAL' ? u.actorType === 'HNX' : u.actorType === 'ORGANIZATION' || u.actorType === 'EXTERNAL',
  );
      ...
      <div className="w-full lg:w-1/2 flex flex-col justify-between px-6 sm:px-12 lg:px-20 py-10">
        <div className="w-full max-w-md mx-auto lg:mx-0 lg:ml-auto lg:mr-24">
          <div className="flex items-center gap-2 mb-10">
            <img src={hnxLogo} alt="Sở Giao dịch Chứng khoán Hà Nội" className="h-9 w-auto" />
          </div>
          ...
          {stage === 'CREDENTIALS' && (
            <div className="inline-flex rounded-md bg-slate-100 p-1 mb-6">
              <button
                type="button"
                onClick={() => setUserType('INTERNAL')}
                className={`px-4 py-2 rounded-[5px] text-xs font-semibold transition-colors ${
                  userType === 'INTERNAL' ? 'bg-hnx-gradient text-white shadow-sm' : 'text-slate-500 hover:text-slate-700'
                }`}
              >
                Người dùng nội bộ
              </button>
              <button
                ...
                className={`px-4 py-2 rounded-[5px] text-xs font-semibold transition-colors ${
                  userType === 'EXTERNAL' ? 'bg-hnx-gradient text-white shadow-sm' : 'text-slate-500 hover:text-slate-700'
                }`}
              >
            ...
            <form onSubmit={submitCredentials} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder={userType === 'INTERNAL' ? 'Nhập tên đăng nhập' : 'Nhập tên đăng nhập hoặc CCCD/MST'}
                />
              </div>
          ...
          {stage === 'CREDENTIALS' && (
            <div className="mt-8 pt-5 border-t border-slate-100">
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2">
                Tài khoản thử nghiệm ({userType === 'INTERNAL' ? 'nội bộ' : 'bên ngoài'})
              </p>
